...
# UPDATED: Changed output directory to reflect MobileNetV3-Small
OUTPUT_DIR = '/home/bt/Desktop/Bee/soybean/output/mobilenet_v3_small_10fold'
BATCH_SIZE = 32
NUM_EPOCHS = 20
NUM_CLASSES = 8
NUM_FOLDS = 10
...
print(f"Starting {NUM_FOLDS}-Fold Cross Validation [MobileNetV3-Small] on {device}...")
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(full_dataset)):
    ...
    # UPDATED: Initialize MobileNetV3-Small Model
    # Using weights='DEFAULT' fetches the best available ImageNet weights
    model = models.mobilenet_v3_small(weights='DEFAULT')
    
    # UPDATED: MobileNetV3-Small's final linear layer is at index 3
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch_val_acc > best_fold_acc:
            ...
            torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'mobilenet_v3_small_fold{fold+1}_best.pth'))
            if epoch_val_acc > overall_best_acc:
                overall_best_acc = epoch_val_acc
                torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, 'mobilenet_v3_small_model.pth'))
...
# UPDATED: File save names
results_df.to_csv(os.path.join(OUTPUT_DIR, 'mobilenet_v3_small_results.csv'), index=False)

history_df = pd.DataFrame(history_list, columns=['Fold', 'Epoch', 'Train Loss', 'Train Acc', 'Val Loss', 'Val Acc', 'Time (s)'])
# UPDATED: File save names
history_df.to_csv(os.path.join(OUTPUT_DIR, 'training_history_mobilenet_v3_small.csv'), index=False)
