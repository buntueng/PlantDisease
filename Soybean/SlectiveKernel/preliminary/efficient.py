...
OUTPUT_DIR = '/home/bt/Desktop/Bee/soybean/modelv3/output/efficientnet_v2_s_10fold'
BATCH_SIZE = 32
NUM_EPOCHS = 20
NUM_CLASSES = 8
NUM_FOLDS = 10
...
print(f"Starting {NUM_FOLDS}-Fold Cross Validation [EfficientNetV2-S] on {device}...")
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(full_dataset)):
    ...
    # UPDATED: Initialize EfficientNetV2-S Model
    # Using weights='DEFAULT' fetches the best available ImageNet weights
    model = models.efficientnet_v2_s(weights='DEFAULT')
    
    # UPDATED: EfficientNetV2-S final linear layer is at index 1 of the classifier block
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch_val_acc > best_fold_acc:
            ...
            torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'efficientnet_v2_s_fold{fold+1}_best.pth'))
            if epoch_val_acc > overall_best_acc:
                overall_best_acc = epoch_val_acc
                torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, 'efficientnet_v2_s_model.pth'))
...
# UPDATED: File save names
results_df.to_csv(os.path.join(OUTPUT_DIR, 'efficientnet_v2_s_results.csv'), index=False)

history_df = pd.DataFrame(history_list, columns=['Fold', 'Epoch', 'Train Loss', 'Train Acc', 'Val Loss', 'Val Acc', 'Time (s)'])
# UPDATED: File save names
history_df.to_csv(os.path.join(OUTPUT_DIR, 'training_history_efficientnet_v2_s.csv'), index=False)
