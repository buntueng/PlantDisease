...
# UPDATED: Changed output directory to reflect EfficientNetV2-S + SKNet
OUTPUT_DIR = '/home/bt/Desktop/Bee/soybean/modelv3/output/efficientnet_v2_s_sknet_10fold'
BATCH_SIZE = 32
NUM_EPOCHS = 20
NUM_CLASSES = 8
NUM_FOLDS = 10
...
class SKAttention(nn.Module):
    def __init__(self, channel=1280, branches=2, reduction=16, L=32):
        ...
        # Create multiple branches with different receptive fields
        self.convs = nn.ModuleList([])
        for i in range(branches):
            # i=0 -> dilation=1 (acts like 3x3), i=1 -> dilation=2 (acts like 5x5)
            self.convs.append(
                nn.Sequential(
                    nn.Conv2d(channel, channel, kernel_size=3, padding=1+i, dilation=1+i, groups=32, bias=False),
        ...
        for i in range(branches):
            self.fcs.append(nn.Conv2d(d, channel, kernel_size=1, bias=False))
    ...
    def forward(self, x):
        ...
        # 1. Split: calculate each branch
        conv_outs = [conv(x) for conv in self.convs]
        
        # 2. Fuse: sum all branch outputs and extract global info
        U = sum(conv_outs)
        S = self.gap(U)
        Z = self.fc(S)
        
        # 3. Select: calculate attention weights for each branch
        weights = [fc(Z) for fc in self.fcs]
        weights = torch.stack(weights, dim=1)  # shape: (b, branches, c, 1, 1)
        attention_weights = self.softmax(weights)
        
        # 4. Multiply and combine
        V = sum([attention_weights[:, i, :, :, :] * conv_outs[i] for i in range(self.branches)])
        return V

# --- Custom Model Wrapper ---
class EfficientNetV2_SKNet(nn.Module):
    def __init__(self, num_classes=8):
        super(EfficientNetV2_SKNet, self).__init__()
        ...
        self.sk_attention = SKAttention(channel=1280, branches=2) 
    ...
    def forward(self, x):
        x = self.features(x)
        x = self.sk_attention(x)  # Apply Selective Kernel Attention
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        x = self.classifier(x)
        return x
...
print(f"Starting {NUM_FOLDS}-Fold Cross Validation [EfficientNetV2-S + SKNet] on {device}...")
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(full_dataset)):
    ...
    # UPDATED: Initialize Custom EfficientNetV2-S + SKNet Model
    model = EfficientNetV2_SKNet(num_classes=NUM_CLASSES)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch_val_acc > best_fold_acc:
            ...
            torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'efficientnet_v2_s_sknet_fold{fold+1}_best.pth'))
            if epoch_val_acc > overall_best_acc:
                overall_best_acc = epoch_val_acc
                torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, 'efficientnet_v2_s_sknet_model.pth'))
...
# UPDATED: File save names
results_df.to_csv(os.path.join(OUTPUT_DIR, 'efficientnet_v2_s_sknet_results.csv'), index=False)

history_df = pd.DataFrame(history_list, columns=['Fold', 'Epoch', 'Train Loss', 'Train Acc', 'Val Loss', 'Val Acc', 'Time (s)'])
# UPDATED: File save names
history_df.to_csv(os.path.join(OUTPUT_DIR, 'training_history_efficientnet_v2_s_sknet.csv'), index=False)
