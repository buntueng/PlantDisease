...
# UPDATED: Changed output directory to reflect SqueezeNet
OUTPUT_DIR = '/home/bt/Desktop/Bee/soybean/output/squeezenet_10fold'
BATCH_SIZE = 32
NUM_EPOCHS = 20
NUM_CLASSES = 8
NUM_FOLDS = 10
...
print(f"Starting {NUM_FOLDS}-Fold Cross Validation [SqueezeNet] on {device}...")
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(full_dataset)):
    ...
    # UPDATED: Initialize SqueezeNet Model
    # Using weights='DEFAULT' fetches the best available ImageNet weights
    model = models.squeezenet1_0(weights='DEFAULT')
    
    # UPDATED: SqueezeNet uses a 1x1 Conv2d layer for classification instead of a Linear layer
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch_val_acc > best_fold_acc:
            ...
            torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'squeezenet_fold{fold+1}_best.pth'))
...
# UPDATED: File save names
results_df.to_csv(os.path.join(OUTPUT_DIR, 'squeezenet_results.csv'), index=False)

history_df = pd.DataFrame(history_list, columns=['Fold', 'Epoch', 'Train Loss', 'Train Acc', 'Val Loss', 'Val Acc', 'Time (s)'])
# UPDATED: File save names
history_df.to_csv(os.path.join(OUTPUT_DIR, 'training_history_squeezenet.csv'), index=False)
