...
# UPDATED: Changed output directory to reflect EfficientNetV2-S + SimAM
OUTPUT_DIR = '/home/bt/Desktop/Bee/soybean/modelv3/output/efficientnet_v2_s_simam_10fold'
BATCH_SIZE = 32
NUM_EPOCHS = 20
NUM_CLASSES = 8
NUM_FOLDS = 10
...
class EfficientNetV2_SimAM(nn.Module):
    def __init__(self, num_classes=8):
        super(EfficientNetV2_SimAM, self).__init__()
        # Load base model with default best weights
        base_model = models.efficientnet_v2_s(weights='DEFAULT')
        
        self.features = base_model.features
        self.simam = SimAM() # Inject SimAM after feature extraction
    ...
    def forward(self, x):
        x = self.features(x)
        x = self.simam(x)        # Apply parameter-free attention
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        x = self.classifier(x)
        return x
...
print(f"Starting {NUM_FOLDS}-Fold Cross Validation [EfficientNetV2-S + SimAM] on {device}...")
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(full_dataset)):
    ...
    # UPDATED: Initialize Custom EfficientNetV2-S + SimAM Model
    model = EfficientNetV2_SimAM(num_classes=NUM_CLASSES)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch_val_acc > best_fold_acc:
            ...
            torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'efficientnet_v2_s_simam_fold{fold+1}_best.pth'))
            if epoch_val_acc > overall_best_acc:
                overall_best_acc = epoch_val_acc
                torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, 'efficientnet_v2_s_simam_model.pth'))
...
# UPDATED: File save names
results_df.to_csv(os.path.join(OUTPUT_DIR, 'efficientnet_v2_s_simam_results.csv'), index=False)

history_df = pd.DataFrame(history_list, columns=['Fold', 'Epoch', 'Train Loss', 'Train Acc', 'Val Loss', 'Val Acc', 'Time (s)'])
# UPDATED: File save names
history_df.to_csv(os.path.join(OUTPUT_DIR, 'training_history_efficientnet_v2_s_simam.csv'), index=False)
